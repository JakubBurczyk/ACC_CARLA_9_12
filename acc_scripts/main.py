    ...
    def update_hud_info(self, clock):
        ...
        if self.hero_actor is not None:
            ...
            hero_mode_text = [
                ...
                'NPC Speed:           %3d km/h' % globals.bot_speed,
                'Last known distance:    %3d m' % globals.last_known_distance
            ]
        else:
...
class CarlaSyncMode(object):
    ...
    def __enter__(self):
        ...
        self.frame = self.world.apply_settings(carla.WorldSettings(
            no_rendering_mode= not globals.rendering,
...
def game_loop(args):
    ...
    try:
            ...
            while True:
                ...
                if world.player_agent is not None:
                    ...
                    if radar_distance_list:
                        ...
                        globals.last_known_distance = min(radar_distance_list)
                        globals.distance_time.append(cnt/fps)
                    else:
                        #globals.last_known_distance = -1
                        pass
        ...
        if globals.save_to_csv:
            def list_to_list_list(lst):
                return list(map(lambda el: [el], lst))

            with open('Velocity_over_time.csv', 'w', newline='') as f:
                write = csv.writer(f)
                write.writerow(["Time", "Velocity"])
                l = [globals.time_list, globals.velocity_list]
                write.writerows(zip(*l))

            with open('Distance_over_time.csv', 'w', newline='') as f:
                write = csv.writer(f)
                write.writerow(["Time", "Distance", "Setpoint"])
...
def main():
    ...
    if args.save_plots:
        print("Output plots will be saved")
        globals.save_plots = True
